...
| **allowIPLiteralAddresses**                                                  | `Boolean`           | `false`              | When `true`, allows IP literal addresses in `MAIL FROM` and `RCPT TO` commands per RFC 5321 (e.g., `user@[192.168.1.1]` or `user@[IPv6:2001:db8::1]`). Disabled by default for security. |
