...
The playground only supports attachments with base64-encoded content. Other content sources (file paths, URLs, streams, buffers) are not available in the browser environment. Attachment fields:
- `content` - Base64-encoded string (required)
- `encoding` - Set to `"base64"` (recommended; may be omitted if the content is recognizably base64)
- `filename` - Name of the file (optional, defaults to `"attachment"`)
...
- [Message configuration](/message/) - the reference for every field the playground accepts.
- [Attachments](/message/attachments) - the attachment shapes to try here.
- [Embedded images](/message/embedded-images) - see how `cid:` references change the MIME tree.
- [Stream transport](/transports/stream) - produce the same output from your own code.
- [Testing with Ethereal](/guides/testing-with-ethereal) - send the result to a real inbox.
